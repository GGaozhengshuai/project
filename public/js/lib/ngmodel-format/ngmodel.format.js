...
(function(gobal, angular) {
    angular.module('ngmodel.format', [])
        .constant("modelFormatConfig", {
            ...
            "floatminus": {
                "formatter": function(args, ctrl) {
                    var floatTwo= /^-?\d+(\.\d{0,2})?$/;

                    var modelValue = args.$modelValue,
                        filter = args.$filter;
                    if (floatTwo.test(modelValue) || args.$viewValue=='') {
                        return filter("number")(modelValue, 2);
                    } else {
                        return undefined;
                    }
                ...
                "parser": function(args) {
                    // var val = parseFloat(args.$viewValue.replace(/[^0-9.-]/g, '')),
                    var val = args.$viewValue.replace(/[^0-9.-]/g, ''),
                        ENOB = 3,
                        tempNum = Math.pow(10, ENOB);
                    // return isNaN(val) ? undefined : Math.round(val * tempNum) / tempNum;
                    return isNaN(val) ? undefined : val;
            ...
            "per": {
                "formatter": function(args, ctrl) {
                    ...
                    if (floatTwo.test(modelValue) || args.$viewValue=='') {
                        ...
                        // modelValue = filter("number")(modelValue, 0);
            ...
            function(modelFormatConfig, $filter, $parse) {
                return {
                    ...
                    link: function(scope, element, attrs, ctrl) {
                        ...
                        ctrl.$formatters.push(function(value, ctrl) {
                            return formatter({
                                "$modelValue": value,
                                "$filter": $filter,
                                "$attrs": attrs,
